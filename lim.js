...
function addLog(message, type = "info") {
  logger.log(message, type, (formattedMessage) => {
    transactionLogs.push(formattedMessage);
    if (transactionLogs.length > 19) { 
...
const configSubmitButton = blessed.button({
  ...
  function safeRender() {
    ...
    setTimeout(() => {
      try {
        if (!isHeaderRendered) {
          figlet.text("19Seniman", { font: "Def Leppard" }, (err, data) => {
